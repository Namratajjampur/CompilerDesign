%{
	...
	*/
	...
	int update_variable_value(char *name,char *value,int flag_struct);
...
postfix_expression
	...
	| postfix_expression '.' IDENTIFIER		{
												...
												strcpy($$,temp); 
                                               // fprintf(fptr,"$1 %s  $$ %s\n",$1,$$);
												int value = update_variable_value($$,"",1);
												if(value == -1)
												{
	...
	| postfix_expression INC_OP				{       //printf("84\n");
												...
												if(val)
												{
													...
													char buff[20];
													sprintf(buff,"%f",temp);
													int value = update_variable_value($1,buff,0);
												}}
	...
	| INC_OP primary_expression				{ float val = find_variable_value($2); 
												if(val)
												{
													...
													char buff[20];
													sprintf(buff,"%f",temp);
													int value = update_variable_value($2,buff,0);
												}
											}
...
assignment_expression
	...
	| unary_expression assignment_operator assignment_expression	{
																		...
																		if(not_defined == 0 || ternary_flag == 1 || find_variable_value($1)!=-1)
																		{
																			...
																			{	value = update_variable_value($1,$3,0);
																			...
																			else if(strcmp($2,"+=")==0)
																			{
																				...
																				value = update_variable_value($1,buffer,0);
																				}
																				else{
																					not_defined = 1;
																				FILE *fptr = fopen("error.txt","a");
																			...
																			//int value = update_variable_value($1,$3,0);
																			if(value == -1 )
																			{
																				not_defined = 1;
																				FILE *fptr = fopen("error.txt","a");
...
struct_specifier
	...
	| STRUCT IDENTIFIER '{' nested_struct struct_declaration_list '}'		
													{
														/* struct_flag is to tell the declaration is of structure 
	...
	| STRUCT IDENTIFIER							{
														...
														/* initialized to denote the member used in the struct declaration 															   is struct */
...
struct_declarator
    ...
    | error  { FILE *fptr = fopen("error.txt","a");fprintf(fptr,"error: %d : Cannot initialize within structure declaration", yylineno); fclose(fptr); fprintf(stderr,"error: %d : Cannot initialize in structure declaration", yylineno);yyerrok;}
	;
...
| error SWTCH '(' expression ')' '{' case_statement '}'    { FILE *fptr = fopen("error.txt","a");fprintf(fptr,"error: %d : Swtch is not a recognized keyword( suggestion : SWITCH )", yylineno); fclose(fptr); fprintf(stderr,"error: %d : swtch is not a recognized keyword( suggestion : SWITCH )", yylineno);yyerrok;}
;
constant_expression
: conditional_expression
;
...
int push_my(char *type,char *name,char *value,int flag,int lineno,int size)
{
	...
	else
	{
		...
		//if(0)
		{
			FILE *fptr = fopen("error.txt","a");
			error = 1;
			fprintf(fptr,"Cannot have multiple declaration for same variable %s\n",name);
            fprintf(stderr,"Cannot have multiple declaration for same variable %s : Line number : %d\n",name,yylineno);
            printf("Cannot have multiple declaration for same variable %s : Line number : %d\n",name,yylineno);
			fclose(fptr);
			return -1;	
		}
		else
	...
	//printf("name : %s",name);
		for(j=0;j<index_to_insert;j++)
		{
            ...
            if(flag_struct==1)
            {
                ...
                //printf("%s,%s\n",temp_name,name);
                if(strlen(temp_name)>=strlen(name))
                {char *p = strstr(temp_name,name);
                //printf("%s\n",p);
                if(p && strcmp(p,temp_name)==0)
                {
                   // printf( "%s %s\n", temp_name, name);
                    return 1;
                }
...
void check(char *arg1,char *arg3)
{
	...
	if( !isdigit(arg1[0]) && arg1[0] != '(' )
	{
		...
		if( !flag_operator )
		{
			...
			value = update_variable_value(arg1,"",0);
			if(value == -1)
			{
				not_defined = 1;
				FILE *fptr = fopen("error.txt","a");
	...
	if( !isdigit(arg3[0]) && arg3[0] != '(' )
	{
		...
		{
			//check if varaible in arg1 is present in symbol table or not
			value = update_variable_value(arg3,"",0);
			if(value == -1)
			{
				not_defined = 1;
				FILE *fptr = fopen("error.txt","a");
