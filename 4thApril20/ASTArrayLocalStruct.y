...
%type <string> IDENTIFIER INTEGER_LITERAL FLOAT_LITERAL STRING_LITERAL HEADER_LITERAL CHARACTER_LITERAL array
...
primary_expression
	: IDENTIFIER		{$$ = id($1);}
	| INTEGER_LITERAL	{$$ = con($1);/* printf("CON VALUE%s\n",$$->con.value);*/}
...
init_declarator
	...
	| IDENTIFIER array 									{  char buff[50];
                                                            ...
                                                            buff[strlen(buff)-1]=')';
                                                            buff[strlen(buff)]='\0';
                                                            $$ = id(buff); printf("THE ARRAY LTERALS %s\n",$2);
                                                        }
	;
...
struct_specifier
	...
	| STRUCT IDENTIFIER '{'  struct_declaration_list '}'                        {   //strcpy(struct_members,"\0"); // reinitialize for each such call
                                                                                    printf("struct id {} \n");$$=$4;
                                                                                    printf("MEMBERS LETS CHECK ITTT:%s\n\n", struct_members);
