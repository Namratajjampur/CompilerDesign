...
case_statement
: case_statement_int  
| case_statement {case_end=1;} case_statement_int  
;

case_statement_int
: CASE conditional_expression {if(case_end==1){fprintf(outfile, " L_case%d_%d  : ifFalse %s = %s goto L_case%d_%d \n",label,switch_counter,top(0),pop(),label+1,switch_counter);++label;}else{fprintf(outfile, " L_case%d_%d  : ifFalse %s = %s goto L_case%d_%d \n L_case%d_%d : \n",label,switch_counter,top(0),pop(),label+1,switch_counter,label+1,switch_counter);++label;case_end=0;}} ':' block_item_list 
