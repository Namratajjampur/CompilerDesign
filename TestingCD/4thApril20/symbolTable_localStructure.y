%{
	...
	*/
	...
	int add_struct_member_in_symbol_table(char *struct_name,char *struct_full_name, int flag);
...
declaration
	...
	| type_specifier init_declarator_list ';'	{	 printf("FIND ITTTT TOGTHER: %s %s",$1,$2);
													...
													else if(!strcmp($1,"struct"))
													{
                                                        ...
                                                        add_struct_member_in_symbol_table(token,"",0);
    ...
    else
    {
        ...
        for(i =0;i<=top_stack_for_symbol_tables;i++)
        {
            for(j=0;j<=symbol_table_stack[i].struct_index_to_insert;j++)
            {
                ...
                {printf("FOUND THE STRUCTURE!!!!!!   %s   %d    %d   %d  %d\n\n",symbol_table_stack[i].struct_defined[j].struct_name,symbol_table_stack[i].struct_defined[j].index_to_insert_member,i,j,scope);flag_1=0;
                x=i;y=j;
                break;
        ...
        if(flag_1==1)
        {
            printf("Error, no structure defined");
        }
        else
